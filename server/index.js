...
app.get('/api/models', async (req, res) => {
  try {
    ...
    res.status(500).json({ 
      error: error.message, 
      details: process.env.NODE_ENV === 'production' ? 'See server logs for details' : error.stack,
      hint: error.code === 'AccessDeniedException' ? 'Check AWS Bedrock model access permissions' : undefined
    });
  }
});
